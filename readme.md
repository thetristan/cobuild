...
If rendering a single file or string, result will contain the output of that render. If rendering multiple files, result will contain an array of objects indicating the status of each file.

'''
Cobuild.OK     # File built successfully

Cobuild.COPY   # File copied

Cobuild.SKIP   # File skipped (existing source and destination 
               # modification times matched; no build needed)

Cobuild.ERR    # Error building this file
'''
...
The only three build-related options are callbacks that are ran on pre and post processing of content, and an option to force the rebuild of files (otherwise cobuild checks the source and destinations modification times to determine whether to build the file or not). These options can be overriden on a per-file basis using the `build` method above.
